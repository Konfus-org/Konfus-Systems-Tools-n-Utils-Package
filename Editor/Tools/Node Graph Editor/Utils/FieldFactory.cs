...
namespace Konfus.Tools.NodeGraphEditor
{
    ...
    public static class FieldFactory
    {
        ...
        static FieldFactory()
        {
            ...
            {
                var drawerAttribute =
                    type.GetCustomAttributes(typeof(FieldDrawerAttribute), false).FirstOrDefault() as
        ...
        {
            Type iNotifyType = typeof(INotifyValueChanged<>).MakeGenericType(fieldType);
        ...
        public static INotifyValueChanged<T> CreateFieldSpecific<T>(T value, Action<object> onValueChanged,
            string label)
        {
            INotifyValueChanged<T> fieldDrawer = CreateField(value, label);
            ...
            string label)
        {
            ...
            else
            {
                try
                {
                    MethodInfo createFieldSpecificMethod = createFieldMethod.MakeGenericMethod(fieldType);
                    try
                    {
                        field =
                            createFieldSpecificMethod.Invoke(null, new[] {value, onValueChanged, label}) as
                                VisualElement;
                    }
                    catch
                    {
