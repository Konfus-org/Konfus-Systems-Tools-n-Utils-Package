...
{
    public class GraphView : BaseGraphView
    {
        ...
        public void ClearView()
        {
            ...
            foreach (BaseNode node in ContentContainer.Nodes) node.RemoveFromHierarchy();

            foreach (BasePort port in ContentContainer.Ports) port.RemoveFromHierarchy();

            foreach (BaseEdge edge in ContentContainer.Edges) edge.RemoveFromHierarchy();
        }
        ...
        {
            foreach (BasePort port in ContentContainer.Ports) callback(port);
        }
        ...
        {
            foreach (BaseNode node in ContentContainer.Nodes) callback(node);
        }

        public void ForEachSelectedNodeDo(Action<BaseNode> callback)
        {
            foreach (BaseNode node in ContentContainer.NodesSelected) callback(node);
        }

        public void ForEachSelectedEdgeDo(Action<BaseEdge> callback)
        {
            foreach (BaseEdge edge in ContentContainer.EdgesSelected) callback(edge);
        }
