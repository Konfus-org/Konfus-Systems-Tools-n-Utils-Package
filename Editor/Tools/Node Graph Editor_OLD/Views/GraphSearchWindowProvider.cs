...
namespace Konfus.Tools.Graph_Editor.Views
{
    ...
    public class GraphSearchWindowProvider : ScriptableObject, ISearchWindowProvider
    {
        ...
        private List<SearchTreeEntry> searchTreeEntries = new();
        private List<NodeCreationEntry> nodeEntries = new();
        private Dictionary<string, List<SearchTreeEntry>> nodeEntriesLookup = new();
        ...
        public void ResolveNodeEntries(Func<bool> nodeEnabledCheck)
        {
            ...
            foreach (NodeCreationEntry entry in nodeEntries)
                if (!nodeEntriesLookup.ContainsKey(entry.fullpath))
                {
                    ...
                    nodeEntriesLookup[entry.fullpath].Add(AddEntry(submenus[submenus.Length - 1], nodeEnabledCheck,
                        entry.action, level));
                }
                else
                {
                    ...
                    if (entries.Count > 0)
                    {
                        foreach (SearchTreeEntry nodeEntry in entries) searchTreeEntries.Add(nodeEntry);
        ...
        {
            var entry = new SearchTreeEntry(label, enabledCheck, action) {level = level};
            searchTreeEntries.Add(entry);
            return entry;
        }
