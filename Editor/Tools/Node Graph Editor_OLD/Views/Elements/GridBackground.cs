...
namespace Konfus.Tools.Graph_Editor.Views.Elements
{
    public class GridBackground : VisualElement
    {
        ...
        private void OnAttachEvent(AttachToPanelEvent evt)
        {
            ...
            _mBaseGraphView = target as BaseGraphView;
            if (_mBaseGraphView == null)
                throw new InvalidOperationException("GridBackground can only be added to a GraphView");
            m_Container = _mBaseGraphView.ContentContainer;

            // Listen for Zoom/Pan Changes
            _mBaseGraphView.OnViewTransformChanged += RequestRepaint;
        }
        ...
        private void RequestRepaint(GraphElementContainer contentContainer)
        {
            MarkDirtyRepaint();
        }
        ...
        {
            Rect clientRect = _mBaseGraphView.layout;
