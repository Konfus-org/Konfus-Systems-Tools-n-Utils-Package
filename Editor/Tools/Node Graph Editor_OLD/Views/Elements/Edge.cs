...
namespace Konfus.Tools.Graph_Editor.Views.Elements
{
    public class Edge : BaseEdge
    {
        ...
        private static VisualElement GetCap()
        {
            ...
            if (s_CapPool.Count > 0)
            {
                result = s_CapPool.Pop();
            }
            else
            {
        ...
        protected virtual void UpdateRenderPoints()
        {
            ...
            Vector2 p1 = BaseGraph.ContentContainer.ChangeCoordinatesTo(this, ControlPoints[0]);
            Vector2 p2 = BaseGraph.ContentContainer.ChangeCoordinatesTo(this, ControlPoints[1]);
            Vector2 p3 = BaseGraph.ContentContainer.ChangeCoordinatesTo(this, ControlPoints[2]);
            Vector2 p4 = BaseGraph.ContentContainer.ChangeCoordinatesTo(this, ControlPoints[3]);
        ...
        protected virtual void ComputeControlPoints()
        {
            ...
            if (OutputOrientation == Orientation.Horizontal)
                AssignControlPoint(ref ControlPoints[1], new Vector2(From.x + offset, From.y));
            else
                AssignControlPoint(ref ControlPoints[1], new Vector2(From.x, From.y + offset));

            if (InputOrientation == Orientation.Horizontal)
                AssignControlPoint(ref ControlPoints[2], new Vector2(To.x - offset, To.y));
            else
                AssignControlPoint(ref ControlPoints[2], new Vector2(To.x, To.y - offset));
        ...
        private void ComputeLayout()
        {
            ...
            Rect rect = new(Vector2.Min(to, from), new Vector2(Mathf.Abs(from.x - to.x), Mathf.Abs(from.y - to.y)));
            ...
            float
                margin = EdgeWidth /
                         BaseGraph.CurrentScale; //Mathf.Max(EdgeWidth * 0.5f + 1, k_MinEdgeWidth / Graph.minScale);
            rect.xMin -= margin;
            rect.yMin -= margin;
            rect.width += margin;
            rect.height += margin;
        ...
        {
            if (EdgeWidth <= 0 || BaseGraph == null) return;

            UpdateRenderPoints();
            if (m_RenderPoints.Count == 0) return; // Don't draw anything
            ...
            // float alpha = 1.0f;
            float zoom = BaseGraph.CurrentScale;
        ...
        protected override void OnAddedToGraphView()
        {
            ...
            BaseGraph.OnViewTransformChanged += MarkDirtyOnTransformChanged;
            OnEdgeChanged();
        }
