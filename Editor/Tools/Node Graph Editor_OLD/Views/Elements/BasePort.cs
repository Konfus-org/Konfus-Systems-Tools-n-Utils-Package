...
namespace Konfus.Tools.Graph_Editor.Views.Elements
{
    public class BasePort : VisualElement, IPositionable
    {
        ...
        private static VectorImage PortCircleGraphics
        {
            get
            {
                if (portCircleGraphics == null)
                {
                    ...
                    portCircleGraphics = ScriptableObject.CreateInstance<VectorImage>();
        ...
        public BasePort(Orientation orientation, Direction direction, PortCapacity capacity)
        {
            ...
            ClearClassList();
            // Label
            m_ConnectorText = new Label {pickingMode = PickingMode.Ignore};
            m_ConnectorText.AddToClassList("port-label");
            Add(m_ConnectorText);

            // Cap
            m_ConnectorBoxCap = new VisualElement {pickingMode = PickingMode.Ignore};
            m_ConnectorBoxCap.AddToClassList("port-connector-cap");

            // Box
            m_ConnectorBox = new VisualElement {pickingMode = PickingMode.Ignore};
            ...
            m_Connections = new HashSet<BaseEdge>();
        ...
        public virtual BaseEdge ConnectTo(BasePort other)
        {
            ...
            BaseEdge edge = ParentNode.BaseGraph.CreateEdge();
        ...
        {
            return Capacity == PortCapacity.Multi || !Connected(ignoreCandidateEdges);
        }
        ...
        public bool SameDirection(BasePort other)
        {
            return Direction == other.Direction;
        }

        public bool IsOnSameNode(BasePort other)
        {
            return other.ParentNode == ParentNode;
        }
        ...
        {
            return m_ConnectorBox.LocalToWorld(GetCenter());
        }

        public Vector2 GetCenter()
        {
            return new Rect(Vector2.zero, m_ConnectorBox.layout.size).center;
        }
        ...
        {
            OnPositionChange?.Invoke(new PositionData {element = this});
        }
        ...
        {
            if (ParentNode != null && ParentNode.BaseGraph != null && ParentNode.BaseGraph.IsViewDrag(e))
            {
                ParentNode.BaseGraph.OnDragOffer(e, true);
                return;
            }
            ...
            // Create edge
            BaseEdge draggedEdge = ParentNode.BaseGraph.CreateEdge();
            draggedEdge.SetPortByDirection(this);
            draggedEdge.visible = false;
            ParentNode.BaseGraph.AddElement(draggedEdge);
        ...
        private void OnDropEnter(DropEnterEvent e)
        {
                ...
                typeof(BaseEdge).IsAssignableFrom(dropPayload.GetPayloadType()))
            {
                ...
                // Sanity
                if (dropPayload.GetPayload().Count == 0) throw new Exception("Drop payload was unexpectedly empty");

                // Grab dragged port
                var draggedEdge = (BaseEdge) dropPayload.GetPayload()[0];
        ...
        private void ConnectToPortWithEdge(DropEvent e, BasePort anchoredPort, BaseEdge draggedEdge)
        {
            ...
            if (draggedEdge.IsRealEdge())
            {
                ...
                ParentNode.BaseGraph.OnActionExecuted(Actions.EdgeDelete, draggedEdge);
            }

            // This was a temporary edge, reset it and remove from the graph
            else
            {
                ParentNode.BaseGraph.RemoveElement(draggedEdge);
            }

            // Create new edge (reusing the old deleted edge)
            draggedEdge.Input = inputPort;
            draggedEdge.Output = outputPort;
            ParentNode.BaseGraph.OnActionExecuted(Actions.EdgeCreate, draggedEdge);
        }
