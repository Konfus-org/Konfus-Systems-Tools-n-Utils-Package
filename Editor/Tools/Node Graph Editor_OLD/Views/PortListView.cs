...
namespace Konfus.Tools.Graph_Editor.Views
{
    public class PortListView : ListView
    {
        ...
        public PortListView(SerializedProperty listProperty, PortInfo portListInfo, NodeView nodeView,
            VisualElement container, int index = -1)
        {
            this.nodeView = nodeView;
            this.listProperty = listProperty.Copy();
            this.container = container;
            portInfo = portListInfo;

            var staticHeader = new Label(portListInfo.fieldName);
            ...
            var removeButton = this.Q<Button>("unity-list-view__remove-button");
            removeButton.clickable = null;
            removeButton.clicked += OnRemoveClicked;
            var addButton = this.Q<Button>("unity-list-view__add-button");
        ...
        private void OnItemsRemoved(IEnumerable<int> indices)
        {
            ...
            for (int i = ports.Count - 1; i >= 0; i--)
            {
                ...
                for (int j = edges.Length - 1; j >= 0; j--) edges[j].Disconnect();
        ...
        private void OnRemoveClicked()
        {
            ...
            if (indices.Any()) listProperty.serializedObject.ApplyModifiedProperties();
        ...
        private VisualElement MakeItem()
        {
            ...
            var port = new PortView(portInfo, null);
            ports.Add(port);
        ...
        private void BindItem(VisualElement rowItem, int i)
        {
            ...
            var label = rowItem[AdjustIfReorderable(0)] as Label;
            ...
            void OnConnectionChanged()
            {
                ...
                {
                    label.text = GraphSettingsSingleton.Settings.defaultLabelForPortListItems;
                }
                else
                {
                    // we need to get the foreign nodeview
                    NodeView attachedNodeView =
                        nodeView.controller.graphController.GetNodeView(prop.managedReferenceValue);
                    if (attachedNodeView != null)
                    {
                        //...and retrieve its name, hihi
                        label.text = attachedNodeView.controller.nodeItem.GetName();
                        label.TrackPropertyValue(
                            attachedNodeView.controller.nodeItem.GetNameSerializedProperty().Copy(),
                            (p) => label.text = p.stringValue);
                    }
                }
            }
