...
namespace Konfus.Tools.Graph_Editor.Editor.Manipulators
{
    public class ZoomManipulator : Manipulator
    {
        ...
        {
            var baseGraphView = target as BaseGraphView;
            if (baseGraphView == null) return;

            IPanel panel = (evt.target as VisualElement)?.panel;
            if (panel.GetCapturingElement(PointerId.mousePointerId) != null) return;

            Vector3 position = baseGraphView.ViewTransform.position;
            Vector3 scale = baseGraphView.ViewTransform.scale;
            ...
            Vector2 zoomCenter = target.ChangeCoordinatesTo(baseGraphView.ContentContainer, evt.localMousePosition);
            float x = zoomCenter.x + baseGraphView.ContentContainer.layout.x;
            float y = zoomCenter.y + baseGraphView.ContentContainer.layout.y;
            ...
            baseGraphView.UpdateViewTransform(position, scale);
