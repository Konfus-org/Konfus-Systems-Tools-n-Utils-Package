...
using ContextMenu = Konfus.Tools.Graph_Editor.Views.ContextMenu;
...
namespace Konfus.Tools.Graph_Editor.Editor.Controllers
{
    public class GraphController
    {
        ...
        private InspectorController<Graph> inspector;
        ...
        private Dictionary<object, NodeView> dataToViewLookup = new();
        ...
        public GraphController(VisualElement uxmlRoot, VisualElement root)
        {
            ...
            inspector = new InspectorController<Graph>(uxmlRoot);
            ...
            if (edgeDropMenu == null) edgeDropMenu = EdgeDropMenu.CreateEdgeDropMenu(this);
        }
        ...
        public void OnRename(object obj)
        {
            ...
            {
                var node = graphView.GetFirstSelectedNode() as NodeView;
        ...
        {
            if (evt.button == 1) SearchWindow.Open(contextMenu, graphView);
        }
        ...
        {
            if (dataToViewLookup.ContainsKey(serializedValue)) return dataToViewLookup[serializedValue];
            return null;
        }
        ...
        private void OnSelected(object data = null)
        {
            ...
            {
                var selectedNode = graphView.GetFirstSelectedNode() as NodeView;
        ...
        public void OnCopy(object data = null)
        {
            ...
            {
                var scopedNodeView = node as NodeView;
                if (scopedNodeView != null) nodesToCapture.Add(scopedNodeView);
            });
        ...
        private void PositionNodesRelative(Vector2 viewPosition, List<Node> nodes)
        {
            if (nodes == null || nodes.Count == 0) return;

            var bounds = new Bounds(nodes[0].GetPosition(), Vector3.zero);
            foreach (Node node in nodes) bounds.Encapsulate(node.GetPosition());
        ...
        public void OnDelete(object data = null)
        {
            ...
            graphView.ForEachSelectedEdgeDo((edge) =>
            {
                ...
                var outputPort = edge.GetOutputPort() as PortView;
                outputPort.Reset();
            });
        ...
        {
            Debug.Log("reload");
            if (graphData != null) Load(graphData);
        }
        ...
        {
            if (internalActions.ContainsKey(actionType)) internalActions[actionType](data);
        }
        ...
        public NodeView CreateNewNode(Type nodeType, bool isUtilityNode = false)
        {
            ...
            Node nodeItem = graphData.AddNode(node, isUtilityNode);
        ...
        private void OnAfterGraphCreated(Graph graphData)
        {
            graphView.ClearView();
        ...
        /// </summary>
        /// <param name="graphData"></param>
        private void OnShouldLoadGraph(Graph graphData)
        {
        ...
        /// </summary>
        /// <param name="baseGraph">The graph data that should get loaded.</param>
        public void OpenGraphExternal(Graph baseGraph)
        {
            OnShouldLoadGraph(baseGraph);
        }
        ...
        /// </summary>
        /// <param name="graphData"></param>
        private void Load(Graph graphData)
        {
            ...
            graphView.schedule.Execute(() =>
            {
                ...
                GraphSettings.LastOpenedGraph = graphData;
                ...
                void ForEachNodeProperty(List<Node> nodes, SerializedProperty nodesProperty)
                {
                    // go over every node...
