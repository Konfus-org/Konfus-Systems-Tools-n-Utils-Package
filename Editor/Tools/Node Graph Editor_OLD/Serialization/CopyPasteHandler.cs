...
namespace Konfus.Tools.Graph_Editor.Editor.Serialization
{
    ...
    {
        private List<Node> clones = new();
        private List<NodeDataInfo> originals = new();
        private Dictionary<INode, INode> originalsToClones = new();
        private Graph baseGraphData;
        ...
        public void CaptureSelection(List<NodeView> nodes, Graph rootData)
        {
            ...
            {
                string relativePath =
                    port.boundProperty.propertyPath.Replace(
        ...
        /// </summary>
        public void Resolve(Graph rootData, System.Action<List<Node>> onBeforeAdding,
            System.Action onAfterAdding)
        {
            var originalsCopy = new List<NodeDataInfo>(originals);
            Clear();

            bool isSameData = true;
            if (baseGraphData != null && baseGraphData != rootData) isSameData = false;
            ...
            foreach (NodeDataInfo original in originalsCopy)
                ...
                if (original != null && original.baseNodeItem != null)
                {
                    ...
                    Node deepClone = DeepClone(original.baseNodeItem);
        ...
        private void ResolveExternalReferences(ref List<NodeReference> internalReferences,
            ref List<NodeReference> externalReferences, ref bool isSameData, ref Graph graphData)
        {
        ...
        /// <returns></returns>
        private Node DeepClone(Node original)
        {
            ...
            byte[] serializedNode =
                SerializationUtility.SerializeValueWeak(original, DataFormat.Binary, out unityObjects);
