...
namespace Konfus.Tools.Graph_Editor.Editor.Serialization
{
    ...
    public class PropertyBag
    {
        ...
        public List<PropertyInfo.PropertyInfo> graphPropertiesAndGroups = new();
            ...
            if (nodeAttribute.createInputPort)
                inputPort = new PortInfo(nodeProperty.propertyPath, nodeType,
                    new PortBaseAttribute(nodeAttribute.inputPortName, nodeAttribute.inputPortCapacity,
                        PortDirection.Input), GraphSettingsSingleton.Settings.defaultInputName);
        ...
        private void InitializeAttributebehaviors()
        {
            ...
            int GetPortAttributeBase(ref List<PortInfo> portInfosList, Type type, string portName = null)
            {
                ...
                Debug.Log(
                    $"Invalid Port declaration detected! Be sure to add [SerializeReference] next to the [Output/PortList] attribute. Field: {attributeBag.type}. Related PropertyPath: {currentProperty.propertyPath}");
                return currentProperty.depth;
            }
            ...
            {
                if (attributeBag.type == typeof(string) ||
                    (!typeof(IEnumerable<object>).IsAssignableFrom(attributeBag.type) && !attributeBag.type.IsArray))
                {
                    Debug.Log(
            ...
            int GetGraphDisplayAttribute()
            {
                ...
                {
                    var attrib = (GraphDisplayAttribute) currentAttribute;
        ...
        private void RetrieveAll(SerializedProperty nodeProperty)
        {
            ...
            do
            {
                ...
                if (property.propertyType == SerializedPropertyType.ManagedReference) diveIntoChildren = false;
                //continue;
                if (ignoreFurtherDepth >= 0)
                    if (property.depth > ignoreFurtherDepth)
                        continue;

                if (IsRealArray(property) || IsTypeThatShouldNotEnter(property)) diveIntoChildren = false;
        ...
        private void DoVisibilityExceptions(List<PropertyInfo.PropertyInfo> propertiesIncludingGroups)
        {
            foreach (PropertyInfo.PropertyInfo groupOrProperty in propertiesIncludingGroups)
                if (groupOrProperty.GetType() == typeof(GroupInfo))
                {
                    var groupInfo = (GroupInfo) groupOrProperty;
                    if (groupInfo.graphProperties.Count > 0) DoVisibilityExceptions(groupInfo.graphProperties);
                }
                else
                {
                    var graphProperty = groupOrProperty as GraphPropertyInfo;
        ...
        private int RetrieveCurrentAttributes()
        {
            ...
            if (ignoreFurtherDepth < 0)
            {
                var graphProperty = new GraphPropertyInfo(currentRelativePropertyPath, currentGraphDisplayAttribute);
                ...
                if (graphProperty.graphDisplay.displayType != DisplayType.Hide)
                {
                    ...
                    if (!IsRealArray(currentProperty) && graphProperty.graphDisplay.createGroup &&
