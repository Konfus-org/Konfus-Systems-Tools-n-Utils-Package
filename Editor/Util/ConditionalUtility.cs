...
namespace Konfus.Editor.Utility
{
    public static class ConditionalUtility
	{
		...
		private static void LogFieldNotFound(SerializedProperty property, string field) => 
			Debug.LogWarning($"Conditional Attribute is trying to check field {field} which is not present");
		private static void LogFieldNotFound(UnityEngine.Object owner, string field) => 
			Debug.LogWarning($"Conditional Attribute is trying to check field {field} which is not present");
		public static void LogMethodNotFound(UnityEngine.Object owner, string method) => 
			Debug.LogWarning($"Conditional Attribute is trying to invoke method {method}");
		
		private static bool IsConditionMatch(SerializedProperty property, bool inverse, string[] compareAgainst)
		{
			if (property == null) return true;

			string asString = property.stringValue.ToUpper();
		...
		private static SerializedProperty FindRelativeProperty(SerializedProperty property, string propertyName)
		{
			...
			if (nestedProperty == null)
			{
				...
				Debug.Log("Not supported: " + arrayProp.displayName);
