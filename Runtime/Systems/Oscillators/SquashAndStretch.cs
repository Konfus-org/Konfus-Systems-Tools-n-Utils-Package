...
{
    [SerializeField]
    private bool constantVolume = true;

    [SerializeField, Tooltip("The oscillator that reacts to this object's physics.")]
    private Oscillator oscillator;

    [SerializeField, Tooltip("The conversion factor from the oscillator's local position to the contribution it has on this object's local scale.")]
    private float conversion = 0.15f;
    ...
    private Vector3 CalculateSquashedAndStretchedScale()
    {
        ...
        if (constantVolume) oscillatorContribution = MaintainVolume(rawOscillatorContribution);
    ...
    {
        Vector3 oscillatorContribution = oscillator.transform.localPosition * conversion;
        oscillatorContribution =
            -oscillatorContribution; // Take the negative of the contribution, in order to get the desired effect of squash/stretch.
        return oscillatorContribution;
    }

    /// <summary>
    ///     Maintains the volume of the scale in all axes, such that i.e. a compression in the given vector's axis results in
    ///     expansion in both the perpendicular axes.
    ///     CAUTION: Last I checked, this function is not actually maintaining an exactly constant volume, the maths is a
    ///     little bit off.
    ...
    private Vector3 MaintainVolume(Vector3 primaryDeformation)
    {
        if (primaryDeformation != Vector3.zero)
        {
            ...
            Vector3 totalDeformation = primaryDeformation + 0.5f * (rightCrossSecondaryDeformation +
                                                                    upCrossSecondaryDeformation +
                                                                    forwardCrossSecondaryDeformation);
