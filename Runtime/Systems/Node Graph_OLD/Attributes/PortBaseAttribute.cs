...
namespace Konfus.Systems.Graph.Attributes
{
    ...
    public class PortBaseAttribute : Attribute
    {
        ...
        private static Dictionary<ConnectionPolicy, Func<Type, Type, bool>> _ConnectionPolicybehaviors = new()
        {
            {ConnectionPolicy.Identical, (input, output) => input == output },
            {ConnectionPolicy.IdenticalOrSubclass, (input, output) => input == output || input.IsSubclassOf(output) || input == typeof(INode) || output == typeof(INode)}
        };
        ...
        public PortBaseAttribute(string name = null, Capacity capacity = Capacity.Unspecified,
            PortDirection direction = PortDirection.Unspecified,
            ConnectionPolicy connectionPolicy = ConnectionPolicy.Unspecified)
        {
            if (capacity != Capacity.Unspecified) this.capacity = capacity;
            if (direction != PortDirection.Unspecified) this.direction = direction;

            if (connectionPolicy != ConnectionPolicy.Unspecified) this.connectionPolicy = connectionPolicy;

            this.name = name;
            isValidConnectionCheck = _ConnectionPolicybehaviors[this.connectionPolicy];
        }
    }
}