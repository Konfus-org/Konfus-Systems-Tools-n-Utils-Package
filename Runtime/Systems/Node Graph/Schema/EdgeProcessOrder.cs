...
namespace Konfus.Systems.Node_Graph.Schema
{
    public static class EdgeProcessOrder
    {
        ...
        {
            return
                edges; // Default Behaviour. NOTE: In the future we could add a timestamp so we know when something was first connected for sure
        }
        ...
        public static IList<SerializableEdge> OrderByLIFO(this IList<SerializableEdge> edges)
        {
            return
                edges.Reverse()
                    .ToList(); // Default Behaviour. NOTE: In the future we could add a timestamp so we know when something was first connected for sure
        }
